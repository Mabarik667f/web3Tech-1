...
export default {
    ...
    setup() {
        ...
        const distributorRegistryHook = async () => {
            await store.dispatch('distributorRegistry', formData.value)
            router.push('/profile');
        }

        return {formData, distributorRegistryHook}
    }
}
</script>
...
<template>
        <wave-form @submit.prevent="distributorRegistryHook" class="default-form">
            <template v-slot:header>
                <h1>Зарегистрироваться</h1>
            </template>
            <template v-slot:fields>
                <div v-for="(value, key) in formData" :key="key">
                <div v-if="key === 'regions'" class="form-group">
