...
export default {
    ...
    setup() {
        ...
        const operatorRegistryHook = async () => {
            await store.dispatch('operatorRegistry', formData.value);
            router.push('/profile');
        }

        return {formData, operatorRegistryHook}
    }
}
</script>
...
<template>
    <div>
        <wave-form @submit.prevent=operatorRegistryHook() class="default-form">
